...
2. 是一个 JSON 或 YAML 格式的文本文件，该文件描述 AWS 基础设施，用来创建 堆栈。

3. 包含9个主要部分。Resources 部分是唯一的必需部分。

4. 模板中的某些部分可以任何顺序显示。但是，在构建模板时，使用以下列表中显示的逻辑顺序可能会很有用，因为一个部分中的值可能会引用上一个部分中的值。
...
1. 创建集群
2. 仅限联网，AWS 提供 Fargate支持。
3. EC2 Linux + 联网，需要创建EC2
4. 定义任务
...
5. MediaStore 容器
6. 行为
7. 定义用户请求到CloudFront后，CloudFront将流量打到哪个源上
8. 第一个源被设置为默认的行为
