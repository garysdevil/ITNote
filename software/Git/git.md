...
05. 删除本地分支 `git branch -D 分支名`

06. 如果远程跟踪分支不存在，但本地存在，则删除本地的 `git fetch -p`
...
08. 查看此文件内容修改的详细操作信息 `git blame fileName `

09. 创建新的远程分支并push上去 `git push origin 本地分支名:远程分支名 `
...
14. 重写历史，删除铭感文件 -- 使用 git内置的 filter-branch （速度较慢）
    ...
    2. --all 标志以处理所有分支和标签
    3. head 指定从当前分支的最新一次提交开始遍历 HEAD~30..HEAD 指定从当前分支最近30次提交开始遍历
    4. 强制覆盖 `git push -f` 如果push失败，前往git库->settings->protected Branches修改，去除保护，再重新执行。（去除成功后，务必记得修改回来）

15. 重写历史，删除铭感文件内容 -- 使用 filter-repo
...
2. 查看凭证 git config -l| grep credential.helper

3. 设置凭证 credential.helper = 凭证名字
   git config --global credential.helper 凭证名字

4. 执行完以上命令后，~/.git/config文件里面会多一项配置
   config
...
5. 第一次推送的时候输入账号密码，第二次往后都不需要再输入密码了。
   --global 表示对该用户全局生效。
   --local 默认为--local，只对本项目生效。

6. 或者直接Copy上面配置的内容到 .git/config文件里面即可。
   .git/config\
   .git-credentials

7. 方式二 clone时将账户密码放入源里，单次生效 `git clone http://username:password@git.github.com/username/project.git`

8. 方式三 将账户密码永久放在本地的源里面
...
2. 只包含远程仓库的HEAD分支。
3. 没有远程仓库的tags。
4. 不fetch子仓库(submodules)。
5. --depth=N 只克隆最进N次的提交记录。
